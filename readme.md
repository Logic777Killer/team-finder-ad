...
После заполнения `.env` и настройки базы данных примените миграции:

```bash
python manage.py migrate
```

Для быстрой проверки можно создать тестовые данные:
...
После этого можно запустить сервер разработки:

```bash
python manage.py runserver
```

Теперь проект доступен по адресу [http://localhost:8000/projects/list/](http://localhost:8000/projects/list/). 
Если порт `8000` занят, можно запустить сервер на другом порту:

```bash
python manage.py runserver 127.0.0.1:8001
```

Тогда проект будет доступен по адресу [http://127.0.0.1:8001/projects/list/](http://127.0.0.1:8001/projects/list/).
